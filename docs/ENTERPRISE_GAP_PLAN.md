...
> **创建日期**: 2026-02-04  
> **修订日期**: 2026-02-10 (基于深度代码审查)  
> **目标**: 修复 P0 阻塞问题，将框架提升至企业级就绪  
> **预计工期**: 8-10 周
...
2026-02-10 深度代码审查发现了原始计划未覆盖的 P0/P1 级架构问题。本次修订将这些问题纳入计划，并调整优先级。

**关键发现**:
- Core 包反向引用 OpenAI/Azure，导致 35+ 传递依赖
- ChatMessage 无 ToolCalls，不支持 Native Function Calling
- AgentBase 异常处理丢失 Exception 对象
- Provider 缺少 ILogger/IOptions/重试

---

## 📊 差距总览 (修订)

### 🔴 新发现的 P0/P1 问题 (2026-02-10)

| 优先级 | 编号 | 任务 | 影响 | 工期 | 状态 |
|--------|------|------|------|------|------|
| **P0** | P0-1 | Core 包拆分 (移除 OpenAI/Azure/Serilog/OTel) | 🔴 架构违规 | 1 周 | ⏳ |
| **P0** | P0-2 | Native Function Calling 支持 | 🔴 核心缺失 | 1.5 周 | ⏳ |
| **P0** | P0-3 | 异常保留 (AgentResponse.Exception) | 🔴 生产阻塞 | 0.5 周 | ⏳ |
| **P1** | P1-1 | Provider 基类抽取 + ILogger/IOptions | 🟡 质量问题 | 1 周 | ⏳ |
| **P1** | P1-2 | DI 生命周期修复 (Captive Dependency) | 🟡 运行时 Bug | 0.5 周 | ⏳ |
| **P1** | P1-3 | 流式事件结构化 (StreamingChatEvent) | 🟡 功能缺失 | 1 周 | ⏳ |
| **P1** | P1-4 | Options 启动校验 (IValidateOptions<T>) | 🟡 质量问题 | 0.5 周 | ⏳ |
| **P1** | P1-5 | Roslyn 分析器接入 | 🟡 代码质量 | 0.5 周 | ⏳ |

### 原始计划功能 (已完成)

| 优先级 | 任务 | 影响 | 状态 |
|--------|------|------|------|
| **P5** | MCP 协议支持 | 🔴 关键 - AI 工具互操作标准 | ✅ |
| **P6** | Agent 评估框架 | 🟡 重要 - 量化 Agent 效果 | ✅ |
| **P7** | 图形化工作流 DSL | 🟡 重要 - 可视化编排 | ✅ |
| **P8** | 多模态支持 | 🟢 增强 - Vision/Audio | ✅ |
| **P9** | MS Agent Framework 集成 | 🟢 增强 - 微软生态互操作 | ⏳ |
| **P10** | 文档与案例完善 | 🟡 重要 - 降低采用门槛 | ⏳ |

> **注意**: P5-P8 虽标记为已完成，但底层 LLM 抽象缺少 Function Calling，
> 限制了这些功能在真实场景中的效果。P0-2 修复后，所有模块将自动受益。
...
|------|------|----------|-----------|
| 企业就绪度 | 69% | 85% | 90%+ |
| 测试数量 | 1,906 | 2,000+ | 2,200+ |
| 代码覆盖率 | 72.9% | 76%+ | 80%+ |
| 文档完整度 | 60% | 70% | 90%+ |
| Options 校验覆盖 | 29% | 100% | 100% |
| Core 传递依赖 | ~35 | ≤15 | ≤15 |
...
```
Phase 1 (P0): Core 拆分 → Function Calling → 异常保留 → Provider 基类
     ↓
Phase 2 (P1): DI 修复 → Structured Output → 流式事件 → Options 校验
     ↓
Phase 3 (P2): 架构测试 → 状态持久化 → DTO 不可变 → ReActAgent 清理
     ↓
原始计划: P9 (MS Agent Framework) → P10 (文档完善)
```

详细改动计划见 [ENTERPRISE_ROADMAP.md](ENTERPRISE_ROADMAP.md)。

---

*计划创建于 2026-02-04 | 修订于 2026-02-10 (基于深度代码审查)*
