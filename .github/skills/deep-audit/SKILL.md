...
## Multi-Round Loop（自动循环）

审计以**多轮循环**方式执行，直到无新发现为止。

```
R1 → R2 → R3 → ... → RN（0 findings）→ 终止
```

### 循环规则

1. **每轮选择全新扫描角度** — 不得与之前任何轮次重复
2. **单轮闭环** — 每轮必须走完 Scan → Classify → Fix → Regression Review → Verify → Report 全链路
3. **轮次间自动推进** — 一轮验证通过后，自动 commit 并进入下一轮，无需等待用户确认
4. **终止条件** — 当一轮完整扫描后 **0 个新发现**（排除 deferred/accepted），输出终止报告
5. **终止报告格式**：
...
- 总修复：X 个 bug
- 总新增测试：Y 个
- 最终状态：Build 0/0 | Tests M/M | CSharpier clean
```
...
8. **自动循环** — 验证通过后自动进入下一轮，直到 clean pass 终止
9. **轮次隔离** — 每轮独立 commit，commit message 包含轮次编号（R1/R2/...）
